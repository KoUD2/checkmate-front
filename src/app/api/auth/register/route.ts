...
export async function POST(request: NextRequest) {
  try {
    ...
    const apiUrl = "https://checkmateai.ru/auth/register";
    console.log(`Forwarding register request to: ${apiUrl}`);
    ...
    if (!apiResponse.ok) {
      try {
        ...
        return NextResponse.json(
          {
            message: `Ошибка регистрации: ${apiResponse.status} - ${errorText}`,
          },
        ...
        return NextResponse.json(
          { message: `Ошибка регистрации: ${apiResponse.status}` },
          { status: apiResponse.status }
        );
      }
    }
